...
test('bottom navigation centers every icon in an equal frame', async () => {
  ...
  assert.match(
    source,
    /&__icon \{[\s\S]*?display: grid;[\s\S]*?width: 24px;[\s\S]*?height: 24px;[\s\S]*?place-items: center;[\s\S]*?font-size: 1\.375rem;[\s\S]*?font-variation-settings: 'wght' 500;[\s\S]*?line-height: 1;/
  )
  assert.match(
    source,
