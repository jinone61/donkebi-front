    ...
    <template v-if="activeTab">
      <section class="workspace-intro dk-container">
        <div class="workspace-intro__identity">
          <p class="dk-eyebrow">Crypto · Binance Spot</p>
          <h1 class="dk-serif">Crypto backtest.</h1>
        </div>
        <div class="workspace-intro__note">
          <span class="workspace-intro__status"><i></i> BINANCE READY</span>
          <p>같은 전략을 바이낸스 현물 봉 데이터 위에서 다시 실행합니다.</p>
        </div>
      </section>
      <q-tabs
        v-model="activeTab"
        ...
        <q-tab-panel name="settings" class="q-pa-none">
          <div class="content-container settings-container">
            ...
            <q-form @submit.prevent="runBacktest">
              <q-card flat bordered class="section-card">
                <q-card-section class="section-heading">
                  ...
                  <div class="text-caption text-grey-6">
                    Symbol과 봉 간격, UTC 기간, 초기 Quote 자산을 입력합니다.
                  </div>
                </q-card-section>
                <q-separator />
                  ...
                  >
                    <template #append>
                      <q-icon
                        name="event"
                        class="cursor-pointer"
                        @click.stop="endDateDialog = true"
                      />
                    </template>
                  </q-input>
                  <q-input
                    v-model.number="form.initialQuoteBalance"
                    type="number"
                    label="초기 Quote 잔고"
                    hint="BTCUSDT는 USDT, ETHBTC는 BTC"
                    min="0"
                    step="any"
                    outlined
                    dense
              ...
              <q-dialog v-model="endDateDialog">
                <q-date
                  v-model="form.endAt"
                  mask="YYYY-MM-DD"
                  color="green-5"
              ...
              <q-card flat bordered class="section-card strategy-settings">
                ...
                >
                  <template #header>
                    <q-item-section>
                      <div class="text-h6 text-grey-9">WRSI 집계</div>
                      <div class="text-caption text-grey-6">
                        봉 개수 또는 달력 주간 기준을 선택합니다.
                      </div>
                    </q-item-section>
                    <q-item-section side>
                      <q-badge outline color="grey-7" :label="wrsiModeLabel" />
                    </q-item-section>
                  </template>
                  <q-card-section class="settings-grid">
                    <q-select
                      v-model="form.wrsiMode"
                      :options="WRSI_MODE_OPTIONS"
                      emit-value
                      map-options
                      label="집계 방식"
                      outlined
                    ...
                    />
                    <q-input
                      v-if="form.wrsiMode === 'BAR_COUNT'"
                      v-model.number="form.barCount"
                      type="number"
                      label="집계 봉 수"
                      min="1"
                      step="1"
                      ...
                      color="green-5"
                    />
                    <div v-else class="wrsi-calendar-note">
                      CALENDAR_WEEK 요청에는 barCount를 포함하지 않습니다.
                    </div>
                  </q-card-section>
                </q-expansion-item>
              </q-card>
          ...
          <div class="content-container" v-if="hasResult">
            <q-card flat bordered class="section-card">
              <q-card-section class="section-heading">
                ...
                <div class="text-caption text-grey-6">
                  {{ formatSessionLabel(backtestResult.actualStartDate) }} ~
                  {{ formatSessionLabel(backtestResult.backtestedThroughDate) }}
                </div>
              </q-card-section>
            </q-card>
            ...
            <q-card flat bordered class="section-card">
              <q-card-section class="section-heading">
                ...
                <div class="text-caption text-grey-6">
                  {{ formatSessionLabel(finalPortfolio.date) }} 기준
                </div>
              </q-card-section>
              <q-separator />
              <q-card-section>
                <div class="metric-grid compact-grid">
                  ...
                  <div class="metric-card">
                    ...
                    <div class="metric-value">
                      {{ formatQuantity(finalPortfolio.totalQuantity) }}
                    </div>
                  </div>
                  <div class="metric-card">
            ...
            >
              <q-card-section class="section-heading chart-range-heading">
                ...
                <div class="chart-range-dates text-weight-medium text-grey-8">
                  {{ formatSessionLabel(chartRangeStartDate) }} ~
                  {{ formatSessionLabel(chartRangeEndDate) }}
                </div>
              </q-card-section>
              <q-separator />
              <q-card-section class="chart-range-controls">
                ...
                <q-range
                  ...
                  :max="chartRangeMax"
                  :step="1"
                  :left-label-value="formatSessionLabel(chartRangeStartDate)"
                  :right-label-value="formatSessionLabel(chartRangeEndDate)"
                  color="green-6"
                  label-color="green-7"
                  label
                  label-always
            ...
            <q-card flat bordered class="section-card">
              ...
              >
                <div>
                  <div class="text-h6 text-grey-9">봉별 백테스트 내역</div>
                  <div class="text-caption text-grey-6">
                    날짜를 선택하면 주문·체결·현금 흐름을 확인할 수 있습니다.
                  </div>
                </div>
                <q-badge color="grey-7" :label="`${dailyRows.length}개 봉`" />
              </q-card-section>
                  ...
                  >
                    <template #header>
                      <div class="daily-row daily-desktop-summary">
                        <div class="daily-cell">
                          <span class="mobile-label">봉 시작</span
                          >{{ formatSessionLabel(day.sessionDate) }}
                        </div>
                        <div class="daily-cell">
                    ...
                    <div class="daily-detail bg-grey-1">
                      <div class="detail-section">
                        <div class="detail-title">봉 시작 전 계획</div>
                        <div class="detail-summary">
                          <span
                            >목표 봉
                            {{ formatSessionLabel(day.plan?.targetDate) }}</span
                          >
                        ...
                        <div v-if="day.plan?.buyOrder" class="detail-note">
                          ...
                          {{ formatQuantity(day.plan.buyOrder.quantity) }} ·
                      ...
                      <div class="detail-section">
                        <div class="detail-title">봉 종가 반영 상태</div>
                        <div class="detail-summary">
                          <span>모드 {{ day.state?.mode || '-' }}</span>
                          <span
                            >WRSI {{ formatNumber(day.state?.wrsi, 2) }}</span
                          >
                          <span
                            >RSI
                            {{ formatNumber(day.state?.dailyRsi, 2) }}</span
                          >
                          <span>MA3 {{ formatPrice(day.state?.ma3) }}</span>
                          <span>MA5 {{ formatPrice(day.state?.ma5) }}</span>
                          <span>FI {{ day.state?.fiSign || '-' }}</span>
                        </div>
                      </div>
            ...
            <q-card flat bordered class="section-card">
              ...
              >
                ...
                <div class="row items-center q-gutter-sm">
                  <q-badge
                    color="grey-7"
                    :label="formatSessionLabel(nextPlan.targetDate)"
                  />
                  <q-badge
              ...
              <q-card-section class="q-pt-none">
                ...
                >
                      ...
                      <q-card flat bordered class="mobile-data-card">
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div class="data-value">
                              {{ formatSessionLabel(props.row.buySessionDate) }}
                            </div>
                          </div>
                          <div>
                  ...
                  <template #body-cell-buySessionDate="props">
                    <q-td :props="props">
                      {{ formatSessionLabel(props.value) }}
                    </q-td>
                  </template>
                </q-table>
                <div v-else class="empty-message">
...
const WRSI_MODE_OPTIONS = [
  { label: '봉 개수', value: 'BAR_COUNT' },
  { label: '달력 주간', value: 'CALENDAR_WEEK' }
]
...
const defenseFields = [
  ...
  { key: 'holdingPeriod', label: '최대 보유기간', suffix: '봉', step: 1 },
  ...
  { key: 'maWindow', label: 'MA 기간', suffix: '봉', step: 1 }
]
...
const defaultBacktestEndAt = new Date()

const form = reactive({
  symbol: 'ZECUSDT',
  interval: '4h',
  startAt: subtractCalendarMonths(currentUtcDate(defaultBacktestEndAt), 1),
  endAt: currentUtcDate(defaultBacktestEndAt),
...
const wrsiModeLabel = computed(
  () =>
    WRSI_MODE_OPTIONS.find(option => option.value === form.wrsiMode)?.label ||
    form.wrsiMode
)
...
const nextSellOrders = computed(() =>
  (nextPlan.value.sellOrders || []).map(order => {
    const submittedOrder = (backtestResult.value?.nextOrders || []).find(
      candidate =>
        candidate.tradeSide === 'SELL' && candidate.tier === order.tier
    )

    return {
      ...submittedOrder,
      ...order,
      tradeSide: order.tradeSide || submittedOrder?.tradeSide || 'SELL'
    }
  })
)
...
const validationErrors = computed(() => {
  const errors = []
  if (!form.symbol.trim()) errors.push('거래쌍을 입력해 주세요.')
  if (!form.startAt || !form.endAt)
    errors.push('시작일과 종료일을 입력해 주세요.')
  if (form.startAt && form.endAt && form.startAt >= form.endAt) {
    errors.push('종료일은 시작일보다 늦어야 합니다.')
  }
  if (!isPositiveNumber(form.initialQuoteBalance))
    errors.push('초기 Quote 잔고는 0보다 커야 합니다.')
  if (!isNonNegativeNumber(form.commissionRatePct))
    errors.push('수수료율은 0 이상이어야 합니다.')
  if (!isNonNegativeNumber(form.slippagePct))
    errors.push('슬리피지는 0 이상이어야 합니다.')
  if (form.wrsiMode === 'BAR_COUNT' && !isPositiveInteger(form.barCount)) {
    errors.push('WRSI 집계 봉 수는 양의 정수여야 합니다.')
  }
...
const dailyRows = computed(() =>
  (backtestResult.value?.dailyResults || []).map(day => {
    ...
    const totalQuantity = day.portfolio?.totalQuantity
...
const summaryCards = computed(() => {
  ...
  const throughDate = backtestResult.value?.backtestedThroughDate || '-'
  ...
  return [
    ...
    },
    {
      ...
      value:
        Math.ceil(
          Math.abs(new Date(throughDate) - new Date(initialDate)) /
            (1000 * 60 * 60 * 24)
        ) + '일',
      caption: `${formatSessionLabel(initialDate)} ~ ${formatSessionLabel(throughDate)}`
    },
    {
      label: '총손익',
  ...
  return {
    ...
    datasets: [
      ...
      },
      {
        type: 'line',
        label: '현금',
        data: rows.map(day => toNumber(day.closingCash)),
        yAxisID: 'asset',
        borderColor: '#8d6e63',
        borderWidth: 1.4,
        pointRadius: 0,
      ...
      },
      {
        ...
        label: '보유 수량',
        hidden: true,
        data: rows.map(day => toNumber(day.totalQuantity)),
        yAxisID: 'quantity',
        borderColor: '#7e57c2',
        borderDash: [4, 4],
        borderWidth: 1.2,
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          title(items) {
            return formatSessionLabel(getDailyExecutionTooltipDate(items[0]))
          },
          label(context) {
    ...
    scales: {
      x: {
        ...
        ticks: {
          ...
          font: { size: isMobile ? 10 : 12 },
          callback(value) {
            return formatPriceChartDateLabel(this.getLabelForValue(value))
          }
        }
      },
      y: {
        ...
        ticks: {
          ...
          font: { size: isMobile ? 10 : 12 },
          callback: value =>
            `${formatCompactNumber(value)} ${backtestResult.value?.quoteAsset || ''}`
        },
...
function toUtcTimestamp(value, endOfDay = false) {
  if (!value) return ''
...
function normalizeOrder(order = {}) {
  return {
    ...order,
    quantity: order.quantity ?? order.orderedQuantity ?? 0,
    orderPrice: order.orderPrice ?? order.rawOrderPrice ?? null,
    buySessionDate: order.buySessionDate || order.buySessionAt || null
  }
}
...
function normalizePlan(plan = {}) {
  return {
    ...plan,
    targetDate: plan.targetAt || null,
    buyOrder: plan.buyOrder ? normalizeOrder(plan.buyOrder) : null,
    sellOrders: (plan.sellOrders || []).map(normalizeOrder)
  }
}

function normalizePortfolio(portfolio = {}, quoteAsset = '') {
  return {
    ...portfolio,
    date: portfolio.at || null,
    availableCash: portfolio.availableQuoteBalance,
    currency: portfolio.currency || quoteAsset,
    tiers: portfolio.tiers || []
  }
}

function normalizeBinanceBacktest(data = {}) {
  const quoteAsset = data.quoteAsset || ''

  return {
    ...data,
    initialAvailableCash: data.initialQuoteBalance,
    actualStartDate: data.actualStartAt,
    backtestedThroughDate: data.backtestedThroughAt,
    targetDate: data.nextPlanAt,
    allTimeHigh: data.allTimeHigh
      ? {
          ...data.allTimeHigh,
          sessionDate: data.allTimeHigh.at
        }
      : null,
    finalPortfolio: normalizePortfolio(data.finalPortfolio, quoteAsset),
    dailyResults: (data.barResults || []).map(bar => ({
      ...bar,
      sessionDate: bar.openTime,
      plan: normalizePlan(bar.plan),
      orders: (bar.orders || []).map(normalizeOrder),
      executions: bar.executions || [],
      cash: bar.cash || {},
      portfolio: normalizePortfolio(bar.portfolio, quoteAsset)
    })),
    nextPlan: normalizePlan(data.nextPlan),
    nextOrders: (data.nextOrders || []).map(normalizeOrder)
  }
}
...
function buildPayload() {
  const payload = {
    ...
    startAt: toUtcTimestamp(form.startAt),
    endAt: toUtcTimestamp(form.endAt, true),
  ...
  if (form.wrsiMode === 'BAR_COUNT') {
    payload.wrsiAggregation.barCount = Number(form.barCount)
  }
...
async function runBacktest() {
  ...
  try {
    const { data } = await api.post(BINANCE_BACKTEST_URL, buildPayload())
    backtestResult.value = normalizeBinanceBacktest(data)
    resetChartRange()
    visibleDailyCount.value = DAILY_HISTORY_PAGE_SIZE
    ...
    if (!submitError.value) {
      submitError.value = `백테스트 호출에 실패했습니다: ${error.message}`
    }
...
function formatMoney(value) {
  ...
  if (numericValue === null) return '-'
  const currency =
    finalPortfolio.value.currency || backtestResult.value?.quoteAsset
  const formatted = formatNumber(numericValue, 8)
...
function formatInteger(value) {
  const numericValue = toNumber(value)
  if (numericValue === null) return '-'
  return formatNumber(numericValue, 8)
}

function formatQuantity(value) {
  const formatted = formatInteger(value)
  const asset = backtestResult.value?.baseAsset
  return formatted === '-' || !asset ? formatted : `${formatted} ${asset}`
}
...
function buildDailyExecutionTooltipLines(rows = [], sessionDate) {
  ...
  return [
    ...
    ...(day.executions || []).map(execution => {
      const matchingOrder = (day.orders || []).find(
        order =>
          order.tradeSide === execution.tradeSide &&
          order.tier === execution.tier
      )
      const orderType =
        execution.tradeSide === 'SELL' ? matchingOrder?.orderType : null
      const executionLabel = [execution.tradeSide, orderType, execution.tier]
        .filter(Boolean)
        .join(' ')

      return `${executionLabel} · ${formatPrice(execution.price)} · ${formatQuantity(execution.quantity)}`
    })
  ]
}
